...
router.get('/latest', async (req, res) => {
    try {
        const bankBalance = await BankBalance.findOne({ order: [['createdAt', 'DESC']] });
        const bankWithdrawals = await BankWithdrawal.findAll({ order: [['timestamp', 'DESC']] });
        const stripeCharges = await StripeCharge.findAll({ order: [['date', 'DESC']] });
        const stripeBalance = await StripeBalance.findOne({ order: [['createdAt', 'DESC']] });
...
router.post('/request', async (req, res) => {
    try {
        ...
        const { name, isTrans, pronouns, amountRequested, category, description, neighborhood, socialMedia, contactMethod, contactInfo, receiveMethod } = req.body;
