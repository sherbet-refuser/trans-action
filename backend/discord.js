...
async function verifyDiscordPermissions(channel) {
    if (channel && channel.isTextBased && channel.isTextBased()) {
        ...
        const requiredPermissions = ['SendMessages', 'CreatePublicThreads', 'CreatePrivateThreads', 'SendMessagesInThreads'];
        requiredPermissions.forEach(perm => {
            if (!permissions.has(perm)) {
                console.error(`Missing required Discord permission: ${perm} on channel ${channel.name}`);
            }
        });
...
function updateState(message) {
    ...
    if (count('👀') < 1) {
        ...
        newState = "Vetting";
        nextActionMsg = "React with ✅ to verify this request, or ❌ to mark it as failed verification.";
    } else if (count('❌') >= 1) {
    ...
    if (newState !== currentState) {
        ...
        if (idMatch) {
            ...
            AidRequest.update({ state: newState }, { where: { id: requestId } })
                .then(() => console.log(`AidRequest record updated for Request ID ${requestId} to state ${newState}`))
                .catch(err => console.error('Failed to update AidRequest record:', err));
        }
    }
}
...
function attachCollector(message, threadName) {
    ...
    collector.on('collect', (reaction, user) => {
        ...
        console.log(`Collected ${emoji} (count: ${current}) from ${user.tag} in thread ${threadName}`);
    ...
    collector.on('dispose', (reaction, user) => {
        const emoji = reaction.emoji.name;
        const current = reaction.count;
        console.log(`Removed ${emoji} (count: ${current}) from ${user.tag} in thread ${threadName}`);
        updateState(message);
    });

    collector.on('end', collected => {
        console.log(`Collector for thread ${threadName} ended with ${collected.size} events.`);
    });
}
...
discordClient.once('ready', async () => {
    ...
    try {
        const channel = await discordClient.channels.fetch(config.discord.aidRequestChannelId);
...
async function sendDiscordAidRequest(details) {
    try {
        const channel = await discordClient.channels.fetch(config.discord.aidRequestChannelId);
        if (channel && channel.isTextBased && channel.isTextBased()) {
            const { id, name, isTrans, pronouns, amountRequested, category, description, neighborhood, socialMedia, contactMethod, contactInfo, receiveMethod, userIP, requestReceivedAt } = details;
