...
async function fetchBankData() {
  ...
  try {
    ...
    const processTransaction = async (tx) => {
      ...
      const existing = await BankWithdrawal.findOne({
...
async function processStripeCharges() {
  ...
  for (const charge of charges) {
    const existing = await StripeCharge.findOne({
...
async function runTasks() {
  ...
  if (config.plaid.updatesEnabled) {
    try {
      await processBankData();
    } catch (err) {
      console.error('Error processing bank data:', err);
    }
  } else {
  ...
  if (config.stripe.updatesEnabled) {
    try {
      await processStripeCharges();
    } catch (err) {
      console.error('Error processing stripe charges:', err);
    }
    try {
      await processStripeBalance();
    } catch (err) {
      console.error('Error processing stripe balance:', err);
    }
  } else {
