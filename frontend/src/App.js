...
function App() {
    ...
    const refreshLatestData = () => {
        axios.get(config.api.endpoint + '/latest')
            .then(res => {
    ...
    return (
        <div className="app-container">
            ...
            <nav>
                <button className={view === 'fund' ? 'active' : ''} onClick={() => setView('fund')}>fund</button>
                <button className={view === 'aid' ? 'active' : ''} onClick={() => setView('aid')}>aid</button>
                <button className={view === 'about' ? 'active' : ''} onClick={() => setView('about')}>about</button>
            </nav>
            {view === 'fund' && (
                <Fund latestData={latestData} />
            )}
            {view === 'aid' && (
                <Aid latestRequests={latestData ? latestData.requests : []} refreshData={refreshLatestData} />
            )}
            {view === 'about' && <About />}
        </div>
    );
