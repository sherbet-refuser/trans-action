...
function Balance({ latestData }) {
    ...
    const pending = latestData.stripeBalance && latestData.stripeBalance.pending && latestData.stripeBalance.pending.length > 0
    ...
    return (
        <div>
            <h2>balance</h2>
            <p>
                <span className="total-balance">${bankBalance.amount.toFixed(2)}</span> (+{pending.toFixed(2)} pending)
            </p>
            <p className="last-sync">last sync: {latestData.lastSynced ? new Date(latestData.lastSynced).toLocaleString() : 'n/a'}</p>
        </div>
    );
}
        ...
        <div>
            ...
            <div className="transactions-grid">
                {paginated.map((item, idx) => {
                    ...
                    const amount = isWithdrawal ? `$${Math.abs(item.amount).toFixed(2)}` : `$${(item.amount / 100).toFixed(2)}`;
                    const type = isWithdrawal ? 'withdrawal' : 'contribution';
                    const typeClass = isWithdrawal ? 'transaction-withdrawal' : 'transaction-contribution';
                    const uniqueKey = (item.id || item.stripeId || 'noId') + `-p${page}-i${idx}`;
                    return (
